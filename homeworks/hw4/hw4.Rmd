...
wrapper <- function(train.data, train.response,
  ...
  fitt <- randomForest(response ~ .,
                       data=train.data, ...)
  
  t <- mean(replicate(runs.number, RMSE(fitt, train.data)))
  v <- mean(replicate(runs.number, RMSE(fitt, test.data)))
  c(t, v)
}
```
...
```{r}
nsize <- data.frame(matrix(unlist((lapply(1:40, function(x) c(x, wrapper(train, train_response, valid, valid_response, ntree=100, mtry=10, nodesize=x, sampsize=5, replace=T))))),ncol = 3, byrow=T))
tr <- nsize[, 1:2]
...
```{r}
mtry <- data.frame(matrix(unlist((lapply(1:10, function(x) c(x, wrapper(train, train_response, valid, valid_response, ntree=100, mtry=x, nodesize=5, sampsize=5, replace=T))))),ncol = 3, byrow=T))
tr <- mtry[, 1:2]
...
Ну, при увеличении mtry результаты ухудшаются для обоих выборок. Это вроде как не переобучение.
...
cross.results <- apply(cross.validation, 1, function(test.sample){
  ...
  # calculating RMSE for every part and default random forest
  return(wrapper(train.data, train.response, test.data, test.response, runs.number=100, ntree=100, mtry=4, nodesize=5, sampsize=5, replace=T))
})
